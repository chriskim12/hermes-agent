"""Public-only URL policy for the insane-search adapter."""
...
class PolicyDecision:
    ...
    def to_boundary(self) -> PublicBoundary:
        return PublicBoundary(
            public_only=True,
            private_url=self.verdict is RetrievalVerdict.SSRF_BLOCKED,
            credentials_used=False,
            auth_or_paywall=self.verdict in {RetrievalVerdict.AUTH_REQUIRED, RetrievalVerdict.PAYWALL},
...
def policy_stop_result(url: str, decision: PolicyDecision) -> RetrievalResult:
    return RetrievalResult(
        ...
        confidence=1.0,
        public_boundary=decision.to_boundary(),
