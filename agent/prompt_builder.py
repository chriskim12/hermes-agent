...
KANBAN_GUIDANCE = (
    ...
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). No secrets/PII; "
    "run rows are durable. For governed Autopilot cards (`admission_snapshot.ready_contract` "
    "with reviewer_loop.required=true), do NOT use legacy `review-required`; complete "
    "with metadata for worker_done/verifier handoff: changed_files, read_only, "
    "forbidden_side_effects, authority_boundary_confirmed, no_pr_reason, evidence_refs, "
    "tests_run, decisions. If governance is unclear, block with `missing-lifecycle-contract`. "
    "Legacy exception: if a non-governed code change needs human review, drop "
    "metadata into `kanban_comment`, then end with "
