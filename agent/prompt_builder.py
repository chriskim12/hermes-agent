...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only for a genuine worker-side blocker.** If you need a human "
    "decision you cannot infer (missing credentials, UX choice, paywalled "
    "source, peer output you need first), call `kanban_block(reason=\"...\")` "
    "and stop. Don't guess. On reviewer-loop governed tasks this does **not** "
    "final-close the board as blocked; it submits a `blocked_candidate` for "
    ...
    "tokens / raw PII in either field — run rows are durable forever. For "
    "reviewer-loop governed tasks, `kanban_complete` submits a "
    "`ready_candidate` to the native review column, not final Done; the "
    "reviewer decides PASS/FAIL/BLOCKED. If you produced a PR/review package, "
    "do **not** block merely because independent review is required — complete "
    "with the PR/check/evidence metadata and let the reviewer judge it.\n"
