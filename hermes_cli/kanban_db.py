...
def _reviewer_loop_max_attempts(closeout_evidence: Any, metadata: Optional[dict]) -> int:
    ...
    loop_cfg = evidence.get("reviewer_loop") if isinstance(evidence.get("reviewer_loop"), dict) else {}
    for source in (evidence, loop_cfg):
        raw = source.get("max_verification_attempts") or source.get("max_reviewer_attempts") or source.get("max_attempts")
        if raw is None:
            continue
        try:
...
def _reviewer_fail_remediation_pending(
    ...
    *,
    ...
    """True when a reviewer FAIL intentionally requeued the original worker.

    This state is different from a raw ready task with an active PR: the next
    worker must update the existing branch/PR with reviewer remediation, not
    create a duplicate PR.  The dispatcher may therefore respawn the worker even
    when recent-run or active-PR guards would otherwise defer it.
    """
    ...
    try:
        ...
        attempt = 1
    max_attempts = _reviewer_loop_max_attempts(evidence, None)
