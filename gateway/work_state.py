...
WORK_SESSION_DELIVER_RETRY_BUDGET = 1
...
def normalize_native_owner_ingress_packet(packet: Dict[str, Any]) -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "owner": (
            _coalesce_text(packet.get("owner"), _nested_get(packet, "owner", "name"))
            or "hermes"
        ).lower(),
        "executor": (
            _coalesce_text(packet.get("executor"), _nested_get(packet, "executor", "name"))
            or "omx"
        ).lower(),
        ...
        "repo_path": _normalize_path_value(
            _coalesce_text(packet.get("repo_path"), context.get("repo_path"))
        ),
        "worktree_path": _normalize_path_value(
...
def _bounded_reason(value: Any, *, max_chars: int = 500) -> str:
...
def _work_session_visible_text(payload: Dict[str, Any]) -> str:
    pane_snapshot = payload.get("pane_snapshot") if isinstance(payload, dict) else None
    candidates = [
        payload.get("visible_text"),
        payload.get("text"),
        payload.get("message"),
        payload.get("last_line"),
        payload.get("output"),
    ]
    if isinstance(pane_snapshot, dict):
        candidates.extend([
            pane_snapshot.get("visible_text"),
            pane_snapshot.get("text"),
            pane_snapshot.get("last_line"),
            pane_snapshot.get("output"),
        ])
    return "\n".join(str(item) for item in candidates if item).strip()
...
def classify_work_session_action_required(
    ...
    *,
    ...
    """Classify a clawhip action-required/stale packet into bounded owner semantics."""

    if not isinstance(payload, dict):
        payload = {}
    event = normalized_event or _normalize_work_session_event_name(
        str(
            payload.get("event")
    ...
    visible_text = _work_session_visible_text(payload)
    text = visible_text.lower()
    reason_parts: List[str] = []

    pane_alive = payload.get("pane_alive")
    if pane_alive is False or _is_falsy_marker(pane_alive) or "can't find pane" in text or "no such pane" in text:
        return WorkSessionSemanticClassification(
            state="orphaned",
            reason="tmux pane is missing or dead",
            required_owner_action="Inspect the work session registry and clean up the orphaned watch.",
            should_alert=False,
        )

    if _is_truthy_marker(payload.get("completed")) or re.search(
        r"\b(all tests passed|process exited with code 0|completed successfully|done)\b",
        text,
    ):
        return WorkSessionSemanticClassification(
            state="completed_idle",
            reason="session appears completed and idle",
            required_owner_action="Close or archive the completed work session after verifying evidence.",
            should_alert=False,
        )

    if _is_truthy_marker(payload.get("command_running")) or re.search(
        r"\b(running tests|pytest|npm test|cargo test|building|compiling|installing)\b",
        text,
    ):
        return WorkSessionSemanticClassification(
            state="tool_running",
            reason="session appears to have an active command running",
            required_owner_action="Wait for the running command to finish before intervening.",
            should_alert=False,
        )

    if re.search(r"\b(approval required|permission required|approve or deny|/approve|/deny)\b", text):
        return WorkSessionSemanticClassification(
            state="permission_prompt",
            reason="session is waiting on an approval or permission prompt",
            required_owner_action="Make an explicit approve/deny decision for this exact session.",
            should_alert=True,
        )

    if re.search(r"\b(waiting for user input|should i proceed|need your input|confirm|please respond)\b", text):
        return WorkSessionSemanticClassification(
            state="blocked_on_user",
            reason="session is waiting for user input",
            required_owner_action="Answer the blocking question or provide a bounded continuation decision.",
            should_alert=True,
        )
    ...
    if stale_minutes not in (None, ""):
        reason_parts.append(f"stale for {stale_minutes} minutes")
    elif idle_seconds not in (None, ""):
        reason_parts.append(f"idle for {idle_seconds} seconds")
    if event in {"pane-stale", "pane.stale", "tmux-stale", "tmux.stale", "watch-stale", "watch.stale", "stale"} or "no output for" in text or "stale" in text or reason_parts:
        return WorkSessionSemanticClassification(
            state="stale",
            reason=_bounded_reason("; ".join(reason_parts) or visible_text or "action-required stale session"),
            required_owner_action="Inspect the session and either continue, escalate, or close with evidence.",
            should_alert=True,
        )

    return WorkSessionSemanticClassification(
        state="unknown",
        reason=_bounded_reason(visible_text or "action-required packet did not match known states"),
        required_owner_action="Inspect the packet manually before taking owner action.",
        should_alert=False,
    )
...
SUPERVISOR_ACTION_CONTINUE = "continue_via_clawhip_deliver"
SUPERVISOR_ACTION_WAIT = "wait"
SUPERVISOR_ACTION_ESCALATE = "escalate_handoff"
SUPERVISOR_ACTION_TERMINAL = "terminal_noop"
...
SUPERVISOR_ACTIONABLE_STATES = frozenset({"blocked", "stale", "retry_needed"})
SUPERVISOR_TERMINAL_SESSION_STATES = frozenset({
    "completed",
    "complete",
    "failed",
    "orphaned",
...
def _mapping_value(payload: Any, *names: str) -> Optional[Any]:
    if not isinstance(payload, dict):
        return None
    for name in names:
        if name in payload:
...
@dataclass(frozen=True)
class DelegatedSupervisorDecision:
    """Bounded supervisor decision for one delegated OMX work record/session.

    The decision is intentionally data-only so gateway, clawhip, or tests can
    consume it without triggering side effects.  ``continue_via_clawhip_deliver``
    means the caller may perform exactly one bounded clawhip deliver using the
    supplied ``deliver_request``; all other actions are no-deliver outcomes.
    """

    action: str
    reason: str
    work_id: Optional[str] = None
    deliver_request: Optional[Dict[str, Any]] = None
    attempts_used: int = 0
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "deliver_request": self.deliver_request,
...
def classify_delegated_omx_supervisor_action(
    record: Optional[WorkRecord],
    session: Optional[Dict[str, Any]] = None,
    *,
    max_deliver_attempts: int = 1,
    prompt: Optional[str] = None,
) -> DelegatedSupervisorDecision:
    """Classify a delegated OMX record/session into one bounded supervisor action.

    Outcomes:
    - ``continue_via_clawhip_deliver``: actionable stale/blocked delegated OMX
      work has safe tmux/session metadata and is still under the configured
      deliver-attempt limit.
    - ``wait``: delegated OMX work is live but not currently actionable.
    - ``escalate_handoff``: metadata is missing/ambiguous/unsafe, or the bounded
      deliver limit is exhausted.  The caller should wake the Hermes owner or
      hand off instead of looping.
    - ``terminal_noop``: the record/session is already terminal or is not a live
      Hermes-owned delegated OMX record.
    """
    limit = max(_normalize_nonnegative_int(max_deliver_attempts, default=1), 1)
    session = session if isinstance(session, dict) else None
    if session and (
        _normalize_bool(_mapping_value(session, "ambiguous", "ambiguous_match"))
        or _normalize_nonnegative_int(_mapping_value(session, "matches_count"), default=1) > 1
    ):
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
            reason="ambiguous_work_session",
    ...
    if record is None:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
            reason="missing_work_record",
            attempts_limit=limit,
        )

    work_id = _coalesce_text(getattr(record, "work_id", None))
    if not work_id:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
    ...
    if not (
        ...
        and getattr(record, "executor", None) == "omx"
        and getattr(record, "mode", None) == "delegated"
    ):
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_TERMINAL,
            reason="not_delegated_omx_work",
            work_id=work_id,
            attempts_limit=limit,
        )

    state = str(getattr(record, "state", "") or "").strip().lower()
    if state not in LIVE_STATES or getattr(record, "close_disposition", None) == "close":
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_TERMINAL,
            reason="terminal_work_record",
            work_id=work_id,
            attempts_limit=limit,
        )

    session_state = _coalesce_text(
        _mapping_value(session, "lifecycle_state", "state", "status") if session else None
    )
    if session_state and session_state.strip().lower() in SUPERVISOR_TERMINAL_SESSION_STATES:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_TERMINAL,
            reason="terminal_work_session",
            work_id=work_id,
            attempts_limit=limit,
        )

    if state not in SUPERVISOR_ACTIONABLE_STATES:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_WAIT,
            reason="work_not_actionable",
            work_id=work_id,
            attempts_limit=limit,
        )

    tmux_session = _coalesce_text(
        _mapping_value(session, "tmux_session", "tmuxSession", "session_name") if session else None,
        getattr(record, "tmux_session", None),
    )
    if not tmux_session:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
    ...
    record_tmux = _coalesce_text(getattr(record, "tmux_session", None))
    if record_tmux and tmux_session != record_tmux:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
    ...
    if session is not None and _mapping_value(session, "trusted_auto_watch", "trusted", "hooked") is not None:
        trusted = _normalize_bool(_mapping_value(session, "trusted_auto_watch", "trusted", "hooked"))
        if not trusted:
            return DelegatedSupervisorDecision(
                action=SUPERVISOR_ACTION_ESCALATE,
                reason="untrusted_work_session",
                work_id=work_id,
                attempts_limit=limit,
            )

    attempts = _normalize_nonnegative_int(
        _mapping_value(session, "deliver_attempts", "deliverAttempt", "attempts") if session else 0,
        default=0,
    )
    if attempts >= limit:
        return DelegatedSupervisorDecision(
            action=SUPERVISOR_ACTION_ESCALATE,
            reason="deliver_attempt_limit_reached",
            work_id=work_id,
            attempts_used=attempts,
            attempts_limit=limit,
        )

    provider = _coalesce_text(_mapping_value(session, "provider") if session else None) or "codex"
    provider_session_id = _coalesce_text(
        _mapping_value(session, "provider_session_id", "session_id") if session else None,
        getattr(record, "executor_session_id", None),
    )
    tmux_pane = _coalesce_text(_mapping_value(session, "tmux_pane", "pane") if session else None)
    deliver_request = {
        "provider": provider,
        ...
        "tmux_pane": tmux_pane,
        "repo_path": _coalesce_text(
            _mapping_value(session, "repo_path") if session else None,
            getattr(record, "repo_path", None),
        ),
        "worktree_path": _coalesce_text(
            _mapping_value(session, "worktree_path") if session else None,
            getattr(record, "worktree_path", None),
        ),
        "prompt": prompt or getattr(record, "next_action", None) or "Continue the delegated OMX work.",
        "attempt": attempts + 1,
        "attempts_limit": limit,
    }
    return DelegatedSupervisorDecision(
        action=SUPERVISOR_ACTION_CONTINUE,
        reason="actionable_delegated_omx_work",
        work_id=work_id,
...
class WorkSessionRecord:
    ...
    deliver_retry_budget: int = WORK_SESSION_DELIVER_RETRY_BUDGET
    deliver_proof: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "deliver_retry_budget": int(self.deliver_retry_budget or WORK_SESSION_DELIVER_RETRY_BUDGET),
            "deliver_proof": self.deliver_proof,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "WorkSessionRecord":
        ...
        return cls(
            ...
            deliver_retry_budget=_normalize_positive_int(
                data.get("deliver_retry_budget"),
                fallback=WORK_SESSION_DELIVER_RETRY_BUDGET,
            ),
            deliver_proof=data.get("deliver_proof"),
            deliver_started_at=(
...
""".strip()
    ...
    def _deliver_refusal_reason(
        ...
        retry_budget = _normalize_positive_int(
            getattr(record, "deliver_retry_budget", None),
            fallback=WORK_SESSION_DELIVER_RETRY_BUDGET,
        )
        if int(record.deliver_attempts or 0) >= retry_budget:
            return "deliver_retry_budget_exhausted"
        if record.deliver_status in WORK_SESSION_DELIVER_TERMINAL_STATUSES:
            return "deliver_terminal_status_present"
        if record.deliver_status == "nudged":
            return "deliver_attempt_in_flight"
    ...
    def evaluate_clawhip_deliver_policy(
        ...
        attempt = int(record.deliver_attempts or 0) + 1
        ...
        return {
            ...
            "attempts_remaining_after_dispatch": max(retry_budget - attempt, 0),
            "provider": record.provider,
            "provider_session_id": record.provider_session_id,
            "linear_card_id": record.linear_card_id,
            "lane_id": record.lane_id,
            ...
            "proof": (
                f"clawhip_deliver_policy:{record.provider}:{record.provider_session_id}:"
                f"attempt={attempt}/{retry_budget}:classifier={classification.state}"
            ),
        }
    ...
    def _maybe_deliver_locked(
        ...
        deliver_request["retry_budget"] = _normalize_positive_int(
            getattr(record, "deliver_retry_budget", None),
            fallback=WORK_SESSION_DELIVER_RETRY_BUDGET,
        )
        deliver_request["proof"] = deliver_request.get("proof") or (
            f"clawhip_deliver_policy:{record.provider}:{record.provider_session_id}:"
            f"attempt={deliver_request['attempt']}/{deliver_request['retry_budget']}:"
            f"classifier={classification.state}"
        )
        ...
        record.deliver_proof = str(deliver_request.get("proof") or "")
        ...
        record.deliver_evidence = {
            ...
            "proof": record.deliver_proof,
            "retry_budget": deliver_request["retry_budget"],
        }
    ...
    def resolve_native_owner_ingress_packet(
        ...
        *,
        ...
        if normalized["owner"] != "hermes":
            return {"status": "missing", "reason": "invalid_owner", "selected_by": None, "packet": normalized, "matches": [], "no_broadcast": True}
        if normalized["executor"] not in {"omx", "clawhip"}:
            return {"status": "missing", "reason": "invalid_executor", "selected_by": None, "packet": normalized, "matches": [], "no_broadcast": True}
        ...
        with self._lock:
            ...
            candidates = [
                record
                for record in self._records
                if record.owner == "hermes" and record.executor == "omx" and record.mode == "delegated"
            ]
            if live_only:
            ...
            if normalized["work_id"]:
                ...
                matches = [
                    ...
                    if (normalized["executor_session_id"] and record.executor_session_id == normalized["executor_session_id"])
                    or (normalized["tmux_session"] and record.tmux_session == normalized["tmux_session"])
                ]
            elif normalized["worktree_path"]:
                selected_by = "worktree_path"
                matches = [record for record in candidates if _normalize_path_value(record.worktree_path) == normalized["worktree_path"]]
            elif normalized["repo_path"]:
                selected_by = "repo_path"
                matches = [record for record in candidates if _normalize_path_value(record.repo_path) == normalized["repo_path"]]
            else:
                return {"status": "missing", "reason": "insufficient_native_owner_correlation", "selected_by": None, "packet": normalized, "matches": [], "no_broadcast": True}

            matches = [WorkRecord.from_dict(record.to_dict()) for record in matches]

        if not matches:
            return {"status": "missing", "reason": "missing_or_closed_native_owner_work_record", "selected_by": selected_by, "packet": normalized, "matches": [], "no_broadcast": True}
        if len(matches) > 1:
            return {"status": "ambiguous", "reason": "ambiguous_native_owner_work_resolution", "selected_by": selected_by, "packet": normalized, "matches": [record.to_dict() for record in matches], "no_broadcast": True}
        record = matches[0]
        return {
            "status": "single_match",
            "reason": "eligible",
