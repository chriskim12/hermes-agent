...
class GatewayRunner:
    ...
    async def _maybe_send_generated_tts_reply(
        self,
        event: MessageEvent,
        response: str,
        agent_messages: list,
    ) -> bool:
        if not self._should_send_generated_tts_reply(event, response, agent_messages):
            return False
        sent = await self._send_voice_reply(event, response)
        if sent:
    ...
    def _release_running_agent_state(
        ...
        *,
        ...
        try:
            ...
            logger.debug("Failed persisting running-session checkpoint during release", exc_info=True)
        return True
    ...
    async def _run_agent(
        ...
        try:
            ...
            if session_key:
                ...
                # out. _release_running_agent_state also persists the running
                # session checkpoint after clearing local running-agent state.
