...
def _check_protected_cwd_command(command: str, cwd: str) -> Optional[dict]:
    ...
    if branch_decision.allowed:
        # Task worktree branch (autopilot/* or wt/*) — allow all commands.
        return None

    # 3. Protected canonical, non-task branch → evaluate the command.
    ...
    # 4. Unknown command from protected cwd → allow with note.
    #    Only known-dangerous patterns are blocked; novel inspection
    #    commands (e.g. `cat`, `head`, `which`, `find`) pass through.
    return None
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # protected canonical checkout.  Container backends already returned
    # above; only local/ssh backends reach this point.
    if workdir:
