...
### Public ID namespace allocation

Kanban has two identifiers with different jobs:

- **Internal task id** — the local DB/run key (`t_*` or SQLite task id). Use it for execution internals, worker claims, and local joins.
- **Public work id** — the human/project reference (`PP-NNN`, for example `BO-014` or `DC-014`). Use it in Discord, Git branches, PR titles, docs, and cross-ledger projections.

Every Kanban-native public id belongs to a two-letter namespace registered in `kanban_namespaces`. The registry is the allocation authority map; a syntactically valid prefix is not enough. Namespaces identify domains; ledger governance is namespace-independent substrate shared by every domain.

There is intentionally no `HL` namespace or `HL` legacy alias. Early discussion used "Hermes Ledger" as a control-plane label, but no `HL-*` cards or live `HL` registry row existed when the policy was finalized. Work-ledger mechanics are common rules, not their own namespace.

| Field | Meaning | Admission rule |
|---|---|---|
| `prefix` | Exactly two ASCII letters, normalized uppercase | invalid shape rejects registration/admission |
| `name` | Human-readable domain name | required |
| `status` | `active`, `reserved_legacy`, or `retired` | only `active` can allocate Kanban-native ids |
| `allocation_authority` | Named allocator/approval owner for that domain | required; approval gate for `register_namespace()` |
| `created_at` | Registry timestamp | generated by DB helper |
| `notes` | Compatibility notes, migration aliases, review reference | required in practice for reserved/retired/migration cases |

Initial registry:

| Prefix | Status | Authority | Use |
|---|---|---|---|
| `BO` | `active` | Chris-approved Brain OS namespace | Brain OS / agent operating-system work, including ledger substrate maintenance (`BO-NNN`). |
| `CH` | `reserved_legacy` | Linear only | Historical Chris/Linear issue namespace. Keep `CH-###` as a legacy reference, source payload identifier, idempotency anchor (`linear:CH-###`), or backlink only. Never mint new Kanban-native `CH-###`. |

Chrisland approved active project allocations seeded by `init_db()`:

| Prefix | Domain/project | Status | Allocation authority |
|---|---|---|---|
| `DC` | DailyChingu | `active` | Chris-approved project namespace |
| `WS` | Whystarve | `active` | Chris-approved project namespace |
| `RS` | Risu | `active` | Chris-approved project namespace |

Status semantics:

- `active` — approved namespace. Native admission may allocate the next `PP-NNN` only after live registry lookup confirms exactly one active registry row for that prefix.
- `reserved_legacy` — preserved for compatibility/projection. It can appear in `legacy_refs[]`, `source_payload`, comments, branches, PRs, or docs as a backlink, but cannot create native work.
- `retired` — closed namespace. It remains resolvable for history, but must not be reused unless an explicit migration/alias policy is recorded and approved before reactivation.

Allocation policy:

1. Choose the project/domain first, then request a two-letter prefix. Do not infer a namespace from a title, repo name, tenant, or a convenient unused prefix.
2. `register_namespace()` is the approval gate. The caller must provide `name`, `status`, and `allocation_authority`; `CH` cannot be registered as `active`.
3. Unknown, unregistered, reserved-legacy, retired, duplicate, or ambiguous namespace state fails closed. No fallback to `BO`, no silent allocation, no execution dispatch.
4. Native work uses `BO-NNN` by default or another registered active `PP-NNN` id. Linear-shadow work keeps `CH-###` only as legacy/source evidence and uses `linear:CH-###` idempotency.
5. Retired prefixes are never reused by convenience. Reactivation requires a written migration/alias policy that states how old `PP-NNN` references resolve, which new authority owns allocation, and which approval record authorized the change.
6. A plain public id mention is lookup/status context, not execution ingress. Execution still requires an explicit execution cue plus live preflight.

This policy keeps CH-419/CH-425 consumption safe: they can rely on registry status instead of guessing namespace authority, and they can fail closed before any worker spawn, branch mutation, or projection write.
